...
class DeepseekV4ForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        # Stream LLM weights — do NOT buffer the full checkpoint in a list
        # (that doubles peak RSS on GB10 UMA and OOMs TP=2 Vision).
        params_dict = dict(self.named_parameters())

        def _iter_llm_weights():
            for name, loaded_weight in weights:
                if name.startswith(("vision.", "aligner.")) or name in self._SENTINEL_NAMES:
